...
export const githubView = Node.create({
	...
	addNodeView() {
		return ({ node }) => {
			...
			addIcon.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"><!-- Icon from Akar Icons by Arturo Wibawa - https://github.com/artcoholic/akar-icons/blob/master/LICENSE --><g fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" d="M12 8v4m0 0v4m0-4h4m-4 0H8"/><circle cx="12" cy="12" r="10"/></g></svg>`;
			...
			function renderContent(attrs = details) {
				...
				topicsSpan.className = "px-3 py-2 text-xs font-bold text-blue-800 border border-blue-600 rounded-full topics-view";
